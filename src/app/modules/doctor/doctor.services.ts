...
const getByIdFromDB = async (id: string): Promise<Doctor | null> => {
  ...
  const result = await prisma.doctor.findUnique({
    ...
    include: {
      ...
      schedules: {
        include: {
          schedule: true, // Include Schedule details in the DoctorSchedule relation
        },
      },
      review: true,
    },
...
const updateIntoDB = async (
  ...
  const result = await prisma.$transaction(async transactionClient => {
    ...
    if (specialties && specialties.length > 0) {
      ...
      // Handle deletions
      await Promise.all(
        deleteSpecialties.map(async (deleteDoctorSpeciality: ISpecialties) => {
      ...
      // Handle insertions
      await Promise.all(
        newSpecialties.map(async (insertDoctorSpecialty: ISpecialties) => {
          const existingSpecialties = await transactionClient.doctorSpecialties.findFirst({
            where: {
